...
def index():
    ...
    scores = [model_results[i]['score'] for i in range(len(model_results))]
    ...
    params = [m['acquisition_params'] + m['decision_params'] for m in best_model]
    return render_template('index.html', ids = ids, functions = functions, goals = goals, scores = scores, model_name = model_name, params = params, pseudo_r2 = pseudo_r2)
